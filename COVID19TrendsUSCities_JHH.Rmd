...
#  lastobs <- nrow(datasetone) - 1;
  ...
  lines(logisticpdf(c(1:length(datsetone)),cdffitglobal$ro,cdffitglobal$to)/cdffitglobal$adjust,lty=2,col="pink")
  cdffit <- logisitcfit(datasetone,cdffitglobal$ro,cdffitglobal$to,5,adjini=cdffitglobal$adjust,daysrange=daysrange)
...
#  {
    ...
    {
      lines(logisticpdf(c(1:length(datsetone)),cdffit2$ro,cdffit2$to)/cdffit2$adjust/maxgain,lty=4,col="blue")
    }
#}
  ...
  legend("topleft",
         legend = c("New Fatalities","Lowess","Global Fit","Last Trend Fit","Optimistic Trend Fit"),
