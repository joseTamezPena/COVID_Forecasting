...
#    }
    ...
    peakDates[cn] <- thedates[lastobs]+as.integer(daystopeakrange[2]+0.5);
    text(90,0.75,
         paste("Current Deaths:", sprintf("%3.0f",max(time_covid19Country[names(totaldeaths[countryNumber]),]))),
         cex=0.8)
    text(90,0.7,
         paste("Estimated Deaths in 60 days:", sprintf("%3.0f from %5.0f Total Expected",expfat,totexpfat)),
         cex=0.8)
...
plot(peakDates,estimated100000,pch=NA,xlab="Peak Date",ylab="Fatalities per 100,000 Population",main="Fatalities vs. Peak Date",xlim=c(thedates[1],lastdate+180),sub=paste("The estimated fatalities are @60 days form today. The farther away the peak from",as.character(lastdate),", the less reliable are the estimates"))
