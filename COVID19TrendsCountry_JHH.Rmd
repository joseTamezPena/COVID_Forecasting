...
#  lines(datsetchangen,lty=1)
  datsetchange <- 0.5*(datsetchangep + datsetchangen)
#  lines(datsetchange,lty=2)
...
#  lines(datsetchange,lty=3)
  ...
  cdffit <- logisitcfit(datasetone[daysrange,],-0.075,numdays,5)
  which.max(datsetchange)
...
#    pstartday <- max(1,lastobs - daysWindow)
    ...
    cdffitn <- logisitcfit(datasetone[pdaysrange,],cdffit$ro,cdffit$to,2,cdffit$adjust)
  ...
  if (numdays ==  lastobs)
  {
    ...
    for (endDay in c(2,3,4,5,6,7,9,12,14,16,18,21,23,25,28))
    {
      ...
      if (ndays > 7)
      {
        ...
        cdffit <- logisitcfit(datasetone[daysrange,],-0.1,ndays,5)  
        if (!inherits(cdffit, "try-error"))
        {
