...
#    bootest <- bootstraplogisitcfit(datasetperc,cdffit,n=5,daysrange=daysrange)
    ...
    meanrpo <- median(bootest$rpo)
    meandr <- median(bootest$defratios)
    for (n in 1:250)
    {
      if (bootest$to[n] > 0)
      {
        lines(c(lastobs:maxplotdata),logisticcdf(c(lastobs:maxplotdata),bootest$ro[n],bootest$to[n],bootest$rpo[n])/bootest$defratios[n],lwd=1,lty=8,col="light gray")
      }
      
    }
    lines(c(lastobs:maxplotdata),logisticcdf(c(lastobs:maxplotdata),meanro,meanto,meanrpo)/meandr,lwd=2,lty=8,col="green")
    ...
    medianpdfpredictions <- logisticpdf(c(1:(2*maxplotdata)),meanro,meanto,meanrpo)/meandr;
    
    result <- list(name=mainName,daysRange=daystopeakrange,peak=max(pdfpredictions*totalEsperado),medianPred=medianpdfpredictions)
    return (result)
