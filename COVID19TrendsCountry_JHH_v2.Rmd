...
names(totaldeaths)
par(op)
cn = which(names(totaldeaths) == "Austria")
...
#  lastobs <- nrow(datasetone) - 1;
  ...
  mindef <- which.min(filteredNewFatalities[peakday:lastobs])+peakday-1;
  
  cat(lastobs,":",peakday,":",filteredNewFatalities[peakday],mindef,filteredNewFatalities[mindef],"\n");
...
#  cdffitglobal <- logisitcfit(datasetone,-0.05,peakday,5)
  ...
  lines(logisticpdf(c(1:length(datsetone)),cdffitglobal$ro,peakday,cdffitglobal$rpo)/cdffitglobal$defecitRatio,lty=2,col="blue")
  too <- cdffitglobal$to
  startday <- max(1,(numdays - daysWindow))
  if (numdays < lastobs)
  {
    startday <- max(1,peakday-daysWindow)
    too <- 0.25*cdffitglobal$to + 0.75*peakday
#    if (filteredNewFatalities[mindef] < 0.75*filteredNewFatalities[lastobs])
#    {
#      too <- 0.5*(mindef + cdffitglobal$to)
#      startday <- max(1,as.integer((mindef + startday)/2) )
#      print(c(too,mindef,startday))
#    }
  }
 ...
 if (inherits(cdffit, "try-error"))
  {
    cat("Error\n")
    startday <- max(1,(numdays - 2*daysWindow))
    daysrange <- c(startday:(lastobs))
    cdffit <- logisitcfit(datasetone,cdffitglobal$ro,peakday,2*cdffitglobal$defecitRatio,adjini=2*cdffitglobal$defecitRatio,daysrange=daysrange)
  }
...
#  lines(logisticpdf(c(1:length(datsetone)),cdffit$ro,cdffit$to)/cdffit$adjust,lty=5,col="green")
  ...
  if (daystopeak <= 0)
  {
#    pstartday <- max(1,(lastobs - 2*daysWindow))
#    plastday <- c(startday:(lastobs))
    ...
    cdffitn <- logisitcfit(
      ...
      lines(logisticpdf(c(1:length(datsetone)),cdffitn$ro,cdffitn$to,cdffitn$rpo)/cdffitn$defecitRatio,lty=5,col="purple")

  }
  lines(logisticpdf(c(1:length(datsetone)),cdffit$ro,cdffit$to,cdffit$rpo)/cdffit$defecitRatio,lty=3,col="green")
...
#  {
    ...
    {
      lines(logisticpdf(c(1:length(datsetone)),cdffit2$ro,cdffit2$to,cdffit2$rpo)/cdffit2$defecitRatio/maxgain,lty=4,col="pink")
    }
#}
...
#  datasetOptimist <- datasetone;
  ...
  if (!inherits(cdffit, "try-error"))
  {
    bootest <- bootstraplogisitcfit(datasetone,cdffit,n=250,daysrange=daysrange)
    meanto <- mean(bootest$to)
    meanro <- mean(bootest$ro)
    meanrpo <- mean(bootest$rpo)
    meandr <- mean(bootest$defratios)
    ...
    maxdayloc <- numeric()
    maxloc <- numeric()
    predictedTotalCases <- logisticcdf(c(1:250),meanro,meanto,meanrpo)/meandr;
    ymax <- max(1,max(predictedTotalCases))
    names(predictedTotalCases) <- thedates[1:250]
...
#    lines(datasetone$days[c(1:lastobs)],10*datasetone$newfatalities[c(1:lastobs)],lty=3,lwd=3,col=2)
    for (n in 1:100)
    {
      if (bootest$to[n] > 0)
      {
      lines(c(lastobs:250),logisticcdf(c(lastobs:250),bootest$ro[n],bootest$to[n],bootest$rpo[n])/bootest$defratios[n],lwd=1,lty=8,col="gray")
      }
      
    }
    lines(c(lastobs:250),predictedTotalCases[lastobs:250],lty=2,lwd=3,col="light blue")
    predictedTotalNewCases <- logisticpdf(c(1:250),meanro,meanto,meanrpo)/meandr
...
#    }
    ...
    expfat <- totexpfat*
      logisticcdf(lastobs+60,meanro,meanto,meanrpo)/meandr;
    ...
    {
      expatPeak <- totexpfat*
        logisticcdf(meanto,meanro,meanto,meanrpo)/meandr;
    }
  ...
  if (numdays ==  lastobs)
  {
    ...
    if (!inherits(cdffit2, "try-error"))
    {
      ...
      predictedTotalNewCases <- logisticpdf(c(1:250),cdffit2$ro,cdffit2$to,cdffit2$rpo)/cdffit2$defecitRatio;
      
      predictedTotalCases <- logisticcdf(c(1:250),cdffit2$ro,cdffit2$to,cdffit2$rpo)/cdffit2$defecitRatio;
      lines(predictedTotalCases/maxgain,lty=5,lwd=2,col="pink")
  
      expfat <- covid19_datapopulation[names(totaldeaths[countryNumber]),
                                       "ExpectedFatalities"]*
        logisticcdf(10000,cdffit2$ro,cdffit2$to,cdffit2$rpo)/cdffit2$defecitRatio/maxgain;
      text(90,0.55,
    ...
    for (endDay in c(2,3,4,5,6,7,9,12,14,16,18,21,23,25,28))
    {
      ...
      if (ndays > 7)
      {
        ...
        {
          newcases <- logisticpdf(c(1:250),cdffit3$ro,cdffit3$to,cdffit3$rpo)/cdffit3$defecitRatio;
...
```{r echo = FALSE, results = "hide", dpi=300, fig.height= 5.0, fig.width= 7.0}
  ...
  datsetone <- as.numeric(time_covid19CountryPerc[names(totaldeaths[1]),])
  thedates <- colnames(time_covid19CountryPerc)[datsetone > 0.0]
  thedates <- str_replace(thedates,"X","")
  thedates <- str_replace_all(thedates,"\\.","/")
  thedates <- as.Date(thedates,"%m/%d/%Y")
...
```{r echo = FALSE, results = "hide", dpi=300, fig.height= 4.0, fig.width= 7.0}
...
colors[peakDates >= lastdate] <- "red";
colors[peakDates < lastdate] <- "blue";
plot(peakDates,estimatedMIllon,pch=NA,xlab="Peak Date",ylab="Fatalities per 1,000,000 Population",main="At Peak Fatalities vs. Peak Date",xlim=c(thedates[1],lastdate+60),ylim=c(0,500),sub=paste("The estimated fatalities at peak date"))
...
barplot(estimatedMIllon,horiz=TRUE,xlim=c(0,500),main="Fatalities per Million at Peak Date",las=2,cex.axis = 1.0,cex.names = 0.45)
