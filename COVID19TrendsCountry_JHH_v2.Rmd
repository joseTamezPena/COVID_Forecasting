...
#  cdffitglobal <- logisitcfit(datasetone,-0.05,peakday,5)
  ...
  if (numdays < lastobs)
  {
    startday <- max(1,peakday-3*daysWindow)
#    startday <- 1
      too <- 0.5*cdffitglobal$to + 0.5*peakday
#     too <- 0.75*peakday
    if ((filteredNewFatalities[mindef] < 0.80*filteredNewFatalities[lastobs-daysWindow]) && (filteredNewFatalities[lastobs] > 5.0e-4))
    {
     adjini = 0.5*cdffitglobal$defecitRatio;
      too <- 0.5*(mindef + lastobs)
      startday <- max(startday,mindef)
 ...
 if (inherits(cdffit, "try-error"))
  {
    cat("Error\n")
    startday <- max(1,as.integer(0.90*startday))
    too <- 1.1*too;
    cat(lastobs,":",too,": Start day:",startday,"Last Day:",lastobs,"\n");
    daysrange <- c(startday:lastobs)
    cdffit <- logisitcfit(datasetone,cdffitglobal$ro,too,5*adjini,adjini=1.1*adjini,daysrange=daysrange)
  }
