...
#    numdays <- which.max(filteredNewFatalities)
  ...
  if (numdays < lastobs)
  {
    startday <- max(1,numdays)
    minday <- startday + which.min(filteredNewFatalities[startday:(lastobs)]);
    print(minday)
    if (minday < (lastobs-daysWindow))
    {
      if (0.5*filteredNewFatalities[lastobs] > filteredNewFatalities[minday])
      {
        startday <- minday - daysWindow
        too <- 0.25*startday+0.750*lastobs
        print(too)
      }
    }
  }
...
#  datasetOptimist <- datasetone;
  ...
  if (!inherits(cdffit, "try-error"))
  {
    ...
    names(predictedTotalCases) <- thedates[1:180]
...
```{r echo = FALSE, results = "hide", dpi=300, fig.height= 3.5, fig.width= 7.0}
...
barplot(estimatedMIllon,horiz=TRUE,xlim=c(0,500),main="Fatalities per Million at Peak Date",las=2,cex.axis = 1.0,cex.names = 0.5)
